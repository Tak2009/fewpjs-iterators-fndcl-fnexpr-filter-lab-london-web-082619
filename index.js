...
const drivers = ["Takeshi", "david", "Paul", "una", "Dolice", "dave","takeharu"];
const driver = "Una"
const driverLtrs = "da"
const driverObjs = [{name: "Takeshi", age: 1},{name: "Mikasa", age: 2},{name: "Una", age: 3},{name: "Tanako", age: 4}]
...
const fuzzyMatch = (drivers, driverIni) => {
    let matches = drivers.filter(d => {
        return d.slice(0,2).toLowerCase() === driverIni.toLowerCase()
    })
    console.log(matches)
    return matches
};
...
const matchName = (driverObjs, driver) => {
    let matches = driverObjs.filter(d => {
        return d.name.slice(0,2).toLowerCase() === driver.toLowerCase()
    })
    console.log(matches)
    return matches
};
